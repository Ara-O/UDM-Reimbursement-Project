<template>
    <section class="xl:w-auto mx-10 sm:mx-20 xl:ml-0">
        <span class="flex items-center gap-6 mb-2">
            <h2 class="font-semibold my-0 text-[24px] md:text-[27px]">Reimbursement Claim Information</h2>
        ...
        <form @submit.prevent="emits('move-to-next-section')">
            <div class="flex gap-x-16 gap-y-3 flex-wrap">
                <span>
                    ...
                    <input type="text" name="name" placeholder="Title of Reimbursement"
                        v-model="props.claim.reimbursementName" required
                ...
                <span>
                    ...
                    <input type="text" name="destination-city-state-input" v-model="props.claim.destination"
                        placeholder="Destination-City, State, Country"
                        class="border-[0.5px] h-11 rounded-md border-gray-200 w-72 box-border pl-5 text-xs border-solid shadow-md">
                </span>
            </div>
            <div class="mt-7 flex gap-4">
                <label for="check-if-udmpu-checkbox" class="text-sm font-normal">Check if using UDMPU 11.6 voucher
                    (please
                    attach voucher/log)</label>
                <input type="checkbox" v-model="props.claim.UDMPUVoucher" id="check-if-udmpu-checkbox">
            </div>
            <div class="text-sm mt-7">
                <span>
                    <label for="pickup">Hold for pickup</label>
                    <input type="radio" id="pickup" name="payment-type" value="Hold for Pickup"
                        v-model="props.claim.paymentRetrievalMethod">
                </span>
                <span class="ml-5">
                    <label for="direct-deposit">Direct Deposit</label>
                    <input type="radio" id="direct-deposit" name="payment-type" value="Direct Deposit"
                        v-model="props.claim.paymentRetrievalMethod">
                </span>
            </div>
            <button type="submit"
