        ...
        <form @submit.prevent="addFoapa">
            <div class="flex gap-x-14 flex-wrap">
                <span>
                    ...
                    <select name="foapa" id="foapa" v-model="assignedFoapa.id" @change="assignFoapaName"
...
import { ReimbursementTicket, FoapaStuff, FoapaInput, FoapaInputWithID } from "../../types/types";
...
const assignFoapaName = (event) => {
    ...
    const selectedFoapa = filteredUserFoapas.value.find(foapa => foapa._id === selectedFoapaId);
    assignedFoapa.value.id = selectedFoapaId;
    assignedFoapa.value.foapaNumber = selectedFoapa ? formatUserFoapa(selectedFoapa) : '';
}
