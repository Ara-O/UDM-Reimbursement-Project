...
" placeholder="Search through your reimbursement requests" />
        ...
        <section v-if="reimbursement_has_loaded">
            <span class="inline-block my-4 items-center gap-3 cursor-pointer" @click="goToReimbursementRequestPage">
            ...
            <div>
                ...
                <div class="flex gap-3 flex-wrap" v-if="view === 'Grid View'">
                    <reimbursement-card-grid v-for="request in filtered_reimbursement_data" :request="request"
                        v-if="reimbursement_has_loaded" @user-duplicated-a-request="userDuplicatedARequest"
                ...
                <div class="reimbursement-wrapper-list" v-if="view === 'Table View'">
                    <reimbursement-card-table :requests="filtered_reimbursement_data" v-if="reimbursement_has_loaded"
                        @user-wants-to-delete-request="confirmDelete"
                        @user-duplicated-a-request="userDuplicatedARequest"></reimbursement-card-table>
                    <skeleton height="11rem" width="24rem" v-else></skeleton>
                </div>
...
watch(sort, (sortValue) => {
    ...
    const reimbursement_data_options = reimbursement_data.value.filter((request) => request.reimbursementName
        .toLowerCase()
        .includes(search_field.value.toLowerCase()))


    if (sortValue === 'Cost (Descending)') {
        filtered_reimbursement_data.value = reimbursement_data_options.sort((a, b) => {
...
watch(filter, (newFilter) => {
    ...
    if (newFilter === "Submitted") {
        filtered_reimbursement_data.value = reimbursement_data_options.filter((request) => request.reimbursementStatus === "Submitted")
    }

    if (newFilter === "In Progress") {
        filtered_reimbursement_data.value = reimbursement_data_options.filter((request) => request.reimbursementStatus === "In Progress")
    }
})
...
onMounted(() => {
    ...
    if (preferredView === null) { view.value = "Grid View"; return }

    if (views.includes(preferredView))
