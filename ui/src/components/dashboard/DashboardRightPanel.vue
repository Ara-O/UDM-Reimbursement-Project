<template>
    <section class="box-border px-7 py-5">
        ...
        <div class="mt-4 flex gap-3">
            <Select v-model="search_field" :options="['Grid View', 'List View', 'Table View']" placeholder="Grid View"
                class="w-full md:w-56">
                <template #dropdownicon>
                    <i class="pi pi-table" />
                </template>
            </Select>
            <Select v-model="search_field" :options="['Name', 'Cost', 'Status']" placeholder="Filter"
                class="w-full md:w-56">
                <template #dropdownicon>
                    <i class="pi pi-filter" />
                </template>
            </Select><Select v-model="search_field" :options="['Name', 'Cost', 'Status']" placeholder="Sort"
                class="w-full md:w-56">
                <template #dropdownicon>
        ...
        <section v-if="reimbursement_has_loaded">
            ...
            <div>
                <div class="flex gap-3 flex-wrap">
                    <reimbursement-card v-for="request in reimbursement_data" :request="request"
                        v-if="reimbursement_has_loaded"
                        @user-wants-to-delete-request="confirmDelete"></reimbursement-card>
...
import ReimbursementCard from './ReimbursementCard.vue';
