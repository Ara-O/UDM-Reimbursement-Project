      ...
      <section class="mb-5">
        <article>
          ...
          <h4 class="email-confirmation-text">
            ...
            {{
              userSignupData ? userSignupData.workEmail.trim() : ""
            }}@udmercy.edu. You should receive an email in 1-2 minutes. Please
            check your spam/junk folder if it does not arrive. Note: The
            6-character code is case-sensitive and will expire in 10 minutes.
          </h4>
          <div class="verify-code-section">
            ...
            <h3
              class="text-sm font-normal no-underline hover:underline mt-6 cursor-pointer"
              @click="resendCode"
            >
              Didn't receive a code? Click to resend code
            </h3>
          </div>
        </article>
      </section>
...
onMounted(() => {
  ...
  if (
    userSignupData.value === undefined ||
    userSignupData.value === null ||
    Object.keys(userSignupData.value).length === 0
  ) {
    toast(
...
async function resendCode() {
  try {
    if (tries.value >= 3) {
      toast(
        "You have exceeded the limit of code resends. Please restart the signup process",
        {
          type: TYPE.WARNING,
        }
      );
    ...
    if (
      userSignupData.value?.workEmail &&
      userSignupData.value.employmentNumber
    ) {
      await axios.post(
